...
import client.ComparisonProtocolOnClient;
import client.PaillierOnClient;

public class EncFastHungarianAlgorithm {
	private static final BigInteger NegONE = BigInteger.ONE.negate();	
	private BigInteger Enc_ZERO = BigInteger.ONE;
	private PaillierOnServer mPaillier;
	private ComparisonProtocolOnServer cp_s;
	private PaillierOnClient pc;
	...
	public BigInteger findEncLargest		//Finds the largest element in a positive array.
	(BigInteger[][] array)
	//works for arrays where all values are >= 0.
	{
	...
	private static double IntToDouble(int x) {
		return Double.valueOf(Integer.toString(x));
	}
	...
	public int[][] hgAlgorithm (BigInteger[][] array, String sumType)
	{
	...
	public int hg_step1(int step, BigInteger[][] cost) {
	...
	public int hg_step6(int step, BigInteger[][] cost, int[] rowCover, int[] colCover, BigInteger maxCost)
	{
	...
	public BigInteger findEncSmallestBeta(BigInteger[][] cost, int[] rowCover, int[] colCover, BigInteger maxCost) {
	...
	public BigInteger findEncSmallest(BigInteger[][] cost, int[] rowCover, int[] colCover, BigInteger maxCost) {
		System.out.println("\t[START]\tfindEncSmallest()");
		BigInteger minval = maxCost;
		BigInteger[] tmp1DArray = new BigInteger[cost.length * cost[0].length];
		int index = 0;
	...
	public static void main(String[] args) {
		...
		EncFastHungarianAlgorithm h = new EncFastHungarianAlgorithm(cp_s, ps, pc);
		...
		BigInteger[][] array = new BigInteger[numOfRows][numOfCols];
		h.generateRandomArray(array, "random");	//All elements within [0,1].
		//</UNCOMMENT>
		...
		for (int i=0; i<array.length; i++)
		{
			for (int j=0; j<array[i].length; j++) {
				...
				array[i][j] = ps.Encryption(array[i][j]);
				//System.out.printf("%s\t", array[i][j].toString());											
			}
			System.out.println();
		}
		...
		assignment = h.hgAlgorithm(array, sumType);	//Call Hungarian algorithm.
		double endTime = System.nanoTime();								
		
		System.out.println("The winning assignment (" + sumType + " sum) is:\n");	
		BigInteger sum = h.Enc_ZERO;
		for (int i=0; i<assignment.length; i++)
		{
			//<COMMENT> to avoid printing the elements that make up the assignment
			System.out.printf("array(%d,%d) = %s\n", (assignment[i][0]+1), (assignment[i][1]+1),
					pc.Decryption(array[assignment[i][0]][assignment[i][1]]).toString());
			sum = sum.multiply(array[assignment[i][0]][assignment[i][1]]).mod(pc.nsquare);			
			//</COMMENT>
		}
		
		System.out.printf("\nThe %s is: %s\n", sumType, pc.Decryption(sum).toString());
